    ...
    return (
        ...
        >
            {[0, 1, 2].map((i) => (
                <View
                    key={i}
                    style={{ width: 7, height: 7, borderRadius: 3.5, backgroundColor: colors.primary, opacity: 0.6 }}
                />
            ))}
        </View>
    );
        ...
        >
            {!isUser && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 3, marginLeft: 4 }}>
                    ...
                    <Text style={{ fontSize: fontSize.caption, fontWeight: '600', color: colors.primary }}>
                        {AI_NAME}
                    </Text>
                </View>
            )}
            <View
                style={{
                    maxWidth: '78%',
                    backgroundColor: isUser ? colors.primary : colors.primarySurface,
                    borderRadius: radius.card,
                    ...
                    borderColor: isUser ? 'transparent' : colors.primaryBorder,
                    ...(isUser ? shadow.primary : {}),
                }}
            >
...
export default function ClientChatDetail() {
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
            const response = await sendChatMessage('client', text, [...messages, userMsg]);
    ...
    const emptyComponent = (
        <View style={{ alignItems: 'center', paddingTop: 60, paddingHorizontal: 32, gap: 8 }}>
            ...
            <Text style={{ fontSize: fontSize.tag, color: colors.textMuted, textAlign: 'center', lineHeight: 20 }}>
                Ask SETu AI anything about fitness, nutrition, or your training plan.
            </Text>
        </View>
    );
